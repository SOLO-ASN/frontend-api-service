...
func (s spacesHandler) Query(c *gin.Context) {
	...
	res, err, endCursor, hasNextPage := s.retriever.Query(c, form.Filters, form.First, form.After)
	spacesQueryResponse := spacesQueryResponse(res, endCursor)
...
func spacesQueryResponse(spaces []model.Space, endCursor string, hasNextPage bool) *types.SpacesQueryResponse {
