...
func (s *spacesRetriever) Query(ctx context.Context, request types.SpacesQueryRequest, limit int, after int) (*[]model.Space, int, bool, error) {
	...
	s.db = s.db.Model(space)

	s.db = s.db.Where("name like ?", request.SearchString+"%").Or("name like ?", "%"+request.SearchString).Or("name like ?", "%"+request.SearchString+"%")

	s.db.Count(&count)
	fmt.Println(count)
	fmt.Println(request.VerifiedOnly)
	s.db = s.db.Where("isVerified = ?", request.VerifiedOnly)
