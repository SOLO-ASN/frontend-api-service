...
pageNumber:第几次请求
*/
func (s *spacesRetriever) Query(ctx context.Context, filters []types.Filter, limit int, after int) (*[]model.Space, int, error, bool) {
	...
	db := s.db.WithContext(ctx).Offset(after).Limit(limit)
	after = after + limit
	// 执行查询并获取结果
