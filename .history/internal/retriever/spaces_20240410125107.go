...
func (s *spacesRetriever) Query(ctx context.Context, request types.SpacesQueryRequest, limit int, after int) (*[]model.Space, int, bool, error) {
	...
	s.db = s.db.Model(space).Where("name like ?", request.SearchString+"%").Or("name like ?", "%"+request.SearchString).Or("name like ?", "%"+request.SearchString+"%").Where("isVerified = ?", request.VerifiedOnly)
